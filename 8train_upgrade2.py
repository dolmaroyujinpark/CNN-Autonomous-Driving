...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        img_name = self.labels_df.iloc[idx, 0]  # CSV에서 직접 경로 가져오기
...
# 데이터셋 및 데이터로더 준비
train_dataset = CustomDataset(csv_file='8train.csv', transform=transform)
train_loader = DataLoader(train_dataset, batch_size=16, shuffle=True)


# 모델 정의 (고정된 Conv 및 Linear 레이어)
class NetworkNvidia(nn.Module):
    def __init__(self):
        ...
        self.conv_layers = nn.Sequential(
            ...
            nn.Dropout(0.6)                 # 드롭아웃 적용 (0.5로 설정)
        )
...
# 모델 저장
torch.save(model.state_dict(), 'model_8class_upgrade4.pth')
